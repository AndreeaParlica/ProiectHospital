...
if(isset($_POST['app_search_submit']))
{
  ...
  if($row['fname']=="" & $row['lname']=="" & $row['email']=="" & $row['contact']=="" & $row['doctor']=="" & $row['docFees']=="" & $row['appdate']=="" & $row['apptime']==""){
    echo "<script> alert('Nu s-au gasit rezultate! Insereaza detalii valide'); 
          window.location.href = 'admin-panel1.php#list-doc';</script>";
  }
  else {
    echo "<div class='container-fluid' style='margin-top:50px;'>
    <div class='card'>
    <div class='card-body' style='background-color:#cca000;color:#ffffff;'>
  <table class='table table-hover'>
    ...
    <tbody>";
          ...
          if(($row['userStatus']==1) && ($row['doctorStatus']==1))  
                    ...
                    {
                      $appstatus = "Anulata de catre tine";
                    }

                    if(($row['userStatus']==1) && ($row['doctorStatus']==0))  
                    {
                      $appstatus = "Anulata de catre doctor";
                    }
          echo "<tr>
            <td>$fname</td>
            <td>$lname</td>
    ...
    echo "</tbody></table><center><a href='admin-panel1.php' class='btn btn-light'>Inapoi la dashboard</a></div></center></div></div></div>";
  }
  }
